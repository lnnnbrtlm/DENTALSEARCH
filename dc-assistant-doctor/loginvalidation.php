...
if(isset($_POST['loginsubmit'])){
	...
	$query = "SELECT * FROM user_accounts WHERE status='INACTIVE' AND email ='$email'";
	...
	$query1 = "SELECT email, Password FROM user_accounts where email=? AND password=? LIMIT 1";
	...
	if($stmt->fetch())
		{
        ...
        else*/ if($stmt1->fetch())
            {
				...
				$sql = "SELECT * FROM user_accounts WHERE email = '".$_SESSION['login_admin1']."'";
...
if(isset($_SESSION['login_admin1'])){
	$email = $_SESSION['login_admin1'];
		...
		$sql3 = "SELECT * FROM user_accounts WHERE email = '$email' LIMIT 1";
		$sql4 = "SELECT * FROM user_accounts WHERE email = '$email' AND user_type='Admin' LIMIT 1";
